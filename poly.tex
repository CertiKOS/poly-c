...
final, safety is vacuously true.
...
%
\begin{enumerate}
\item If $v \in \Vrt$ is final, $\forall \state,\, \Phi_v(\state) \ge 0$.
\item If $(v_1, f, \delta, v_2) \in E$ and $\delta < 0$, then $\forall \state, \Phi_{v_1}(\state) \ge 0$.
\item If $(v_1, f, \delta, v_2) \in E$, then $\forall \state, \Phi_{v_1}(\state) \ge \Phi_{v_2}(\state) + \delta$.
\end{enumerate}
%
...
\end{theorem}
\begin{proof}
By induction on the accessibility, using Theorem~1  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
