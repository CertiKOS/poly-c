...
that is similar to usual bytecode formats.  This enables
analysis at a lower-level where precise cost-semantics
are easier to define, and allows resource analysis of
programs for which the source code is not available.
Low-level resource analysis of programs is also desirable
in embedded or critical systems where the resource bounds
(e.g.\ time, stack) need to be valid at the machine
